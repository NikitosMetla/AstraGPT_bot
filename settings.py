...
def initialize_logger():
    ...
    bot_name = "TEST" if is_test_bot else "MAIN"
    ...
    for lvl in ("START_BOT", "STOPPED", "ERROR_HANDLER", "GPT_ERROR"):
        logger.add(
            loguru_sink_wrapper,
            level=lvl,
...
1. Генерация и редактирование изображений (инструмент tool - generate_gemini_image)
2. Поиск актуальной информации (инструмент tool - search_web)
3. Обработка голосовых сообщений и фото
4. Примерка одежды по фото
5. Создание напоминаний (инструмент tool - add_notification) - ОБЯЗАТЕЛЬНО уточняй конкретное время у пользователя перед вызовом, если он его явно не указал
6. Примерка одежды по фото - если пользователь про это спрашивает, скажи ему ввести команду "/try_on", чтобы начать примерку (пользователь должен будет отправить сначала фото модели, потом фото одежды)
...
SUPPORTED_TEXT_FILE_TYPES = (
    ...
    "kt", "scala", "clj", "hs", "ml", "fs", "vb", "pas", "pl", "sh", "bash",

    # Скрипты и конфиги
    "sql", "r", "matlab", "m", "dockerfile", "makefile",

    # Документация и разметка
    "rst", "tex", "latex", "adoc", "org",

    # XML форматы
    "xml", "xsd", "xsl", "xslt", "soap", "wsdl", "rss", "atom",
