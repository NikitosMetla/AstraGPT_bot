...
#             if fmt in _MIME_BY_FORMAT:
#                 return _MIME_BY_FORMAT[fmt], img_bytes
#             out = BytesIO()
...
#
#
# def _build_contents(prompt: str, ref_imgs: List[bytes]) -> List[Union[str, types.Part]]:
#     parts: List[Union[str, types.Part]] = [prompt]
#     for b in ref_imgs:
#         mime, raw = _detect_or_convert_to_png(b)
...
#             raise SafetyBlockedError("Генерация заблокирована правилами безопасности модели.")
#     except IndexError:
#         raise NoImageInResponseError("Модель не вернула кандидатов (вероятно, вход отклонён).")
...
#                 return data
#     raise NoImageInResponseError("В ответе отсутствуют данные изображения (inline_data).")
#
#
...
#     if not key:
#         raise AuthError("Не задан API-ключ (задайте GOOGLE_API_KEY или передайте api_key аргументом).")
#     refs = _normalize_ref_images(reference_images)
#     contents = _build_contents(prompt, refs)
#     client = genai.Client(
#         api_key=key,
