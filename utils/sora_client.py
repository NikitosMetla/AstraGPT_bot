...
class KieSora2Client:
    ...
    async def _poll_task_status(self, task_id: str) -> str:
        ...
        while True:
            ...
            if state == "waiting":
                logger.info(f"Генерация в процессе... ({int(elapsed)}с)")
            ...
            if state == "success":
                ...
                video_url = result_urls[0]  # Берём первый URL
            ...
            # Неизвестный статус
            raise KieSora2Error(f"Неизвестный статус: {state}")
    ...
    async def image_to_video(
        ...
        Args:
            image: Изображение (URL, байты или путь к файлу)
            prompt: Описание желаемого движения/анимации
        ...
        Returns:
            str: URL сгенерированного видео или task_id (если указан callback_url)
        ...
        # Добавляем изображение в input
        if isinstance(image_data, str) and image_data.startswith(("http://", "https://")):
            payload["input"]["image_url"] = image_data  # В input и image_url
        else:
            payload["input"]["image_base64"] = image_data  # В input и image_base64

        if callback_url:
            payload["callBackUrl"] = callback_url  # На верхнем уровне
